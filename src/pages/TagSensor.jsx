...
export default function TagSensor() {
  ...
  // Synthetic feeding event data
  const feedingData = Array.from({ length: 50 }, (_, i) => ({
    time: i,
    pH: i < 10 ? 1.5 + Math.random() * 0.3 : 
        i < 25 ? 2.5 + Math.sin((i - 10) / 3) * 0.8 : 
        1.8 + Math.random() * 0.4,
    nh4: i < 10 ? 0.2 + Math.random() * 0.1 : 
         i < 30 ? 0.5 + Math.sin((i - 10) / 5) * 0.3 :
    ...
    <div className="min-h-screen px-4 py-24">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-xl text-gray-400">Revolutionary Bio-Sensor for Real-Time Feeding Detection</p>
        </motion.div>
  ...
  return (
    ...
    >
      ...
      <div className="grid md:grid-cols-4 gap-6">
        <SpecCard title="Battery Life" value="6-12" unit="months" icon="🔋" color="text-green-400" />
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          <div>
            ...
            <div className="space-y-3 text-gray-300">
              <p>🔬 <strong>Baseline:</strong> Shark stomach pH ≈ 1.0-2.0 (highly acidic)</p>
              <p>🍽️ <strong>Feeding Event:</strong> pH rises rapidly to 2.5-3.5 as food buffers acid</p>
              <p>⏱️ <strong>Recovery:</strong> pH returns to baseline over 30-45 minutes</p>
              <p>📊 <strong>Threshold:</strong> ΔpH ≥ 0.4 within 120s triggers event logging</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3 text-gray-300">
              <p>🧬 <strong>Source:</strong> Protein digestion produces ammonium ions</p>
              <p>🐟 <strong>Fish/Squid:</strong> High NH₄⁺ concentration (protein-rich)</p>
              <p>🦐 <strong>Crustaceans:</strong> Medium NH₄⁺ levels</p>
              <p>📈 <strong>Analysis:</strong> NH₄⁺ AUC (area under curve) classifies prey type</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-700/50 rounded-lg p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={feedingData}>
              ...
              <Line type="monotone" dataKey="nh4" stroke="#10b981" strokeWidth={2} name="NH₄⁺ Concentration" />
            </LineChart>
          </ResponsiveContainer>
    ...
    >
      ...
      <div className="bg-slate-800/50 backdrop-blur-lg border border-blue-500/20 rounded-xl p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          {['Gastric Capsule', 'Inductive Link', 'Dorsal Tag', 'Satellite', 'Cloud Platform'].map((step, idx) => (
            <div key={step} className="flex items-center">
              ...
              {idx < 4 && (
                <div className="text-blue-400 text-2xl mx-4">→</div>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="bg-slate-800/50 backdrop-blur-lg border border-blue-500/20 rounded-xl p-8">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={powerBudget}>
            ...
            <YAxis stroke="#94a3b8" label={{ value: 'Average Power (µW)', angle: -90, position: 'insideLeft', fill: '#94a3b8' }} />
            <Tooltip 
              contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #3b82f6' }}
              labelStyle={{ color: '#e2e8f0' }}
            />
            <Area type="monotone" dataKey="power" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.6} />
          </AreaChart>
        </ResponsiveContainer>
    ...
    >
      ...
      <div className="bg-slate-800/50 backdrop-blur-lg border border-blue-500/20 rounded-xl p-8">
        ...
        <div className="space-y-6">
          {[
            { step: 1, title: 'Capture & Preparation', description: 'Shark captured using safe handling protocols. Gastric capsule activated and calibrated.' },
            { step: 2, title: 'Capsule Insertion', description: 'Capsule inserted into stomach via esophagus. Non-invasive procedure, no anesthesia required.' },
            { step: 3, title: 'Dorsal Tag Attachment', description: 'External dorsal tag attached with inductive receiver coil and satellite uplink.' },
            { step: 4, title: 'Release & Monitoring', description: 'Shark released. System begins baseline monitoring and waits for feeding events.' },
            { step: 5, title: 'Data Collection', description: 'Feeding events transmitted to dorsal tag, uplinked to satellite on surfacing.' },
            { step: 6, title: 'Natural Exit', description: 'Capsule designed for gastro-retention during mission, natural exit after 6-12 months.' }
          ].map((item) => (
        ...
        <div className="mt-6 bg-gradient-to-r from-blue-600/20 to-green-600/20 border border-blue-500/30 rounded-lg p-6">
          <div className="text-center">
            ...
            <p className="text-gray-300">
              Tag feeding data validates and refines SFI model predictions → Improved foraging hotspot forecasts
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="bg-slate-700/50 rounded-lg p-6">
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1">
          ...
          <div className="inline-block bg-green-600/20 text-green-400 text-xs px-2 py-1 rounded">{status}</div>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="bg-slate-700/50 rounded-lg p-6 text-center">
      ...
      <div className="space-y-2 text-sm">
        <div><span className="text-gray-400">Size:</span> <span className="text-white">{size}</span></div>
        <div><span className="text-gray-400">Habitat:</span> <span className="text-white">{habitat}</span></div>
        <div><span className="text-gray-400">Foraging:</span> <span className="text-white">{foraging}</span></div>
      </div>
    </div>
  )
}
